...
# choose data files
spectra_path = os.getcwd()+os.sep+"raw_data"+os.sep+"ex5_sim_H25keV32deg_LiW"
...
for f in range(0, len(datas)):    
    ...
    Au_peak_pos = 1000 #approximate
    E1_FWHM = 0
    E2_FWHM = 0
    for E in range (1000, int(spectrum_en[-1])):            
    ...
    width = Au_peak_pos/1000

    calib_coeff = float(datas[f].split("n")[0].split("i")[-1])/width
    width *=calib_coeff 
...
SCD.create_conc_plots(datas, data_cnv, data_simple_deconv, data_numeric_deconv, type="thickness",
                      conc_element_name="Li", y_max=101, y1_step=10, error_max=5) 
