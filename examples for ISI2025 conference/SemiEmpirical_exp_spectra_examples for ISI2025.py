...
for spectrum in exp_spectra:
    if not "ref" in spectrum and ".txt" in spectrum :
        ...
        # Basic semireference method with one reference (deprecated)
       ...
       # Semi empirical method based on Young formula
        ...
        if do_spectra_charts and plot_ions in data.incident_atom:
            ...
            if "Ne" in data.incident_atom:
                ...
                plt.plot(data.spectrum_en[int(Emin/leis.step):int(Emax/leis.step)]/1000, Au_Deconv_spectrum+Pd_Deconv_spectrum, "m-.", label=t("legend_sum"), linewidth=4, alpha=0.7)
            ...
            if "Ne" in data.incident_atom:
                ne_data.append(conc_Au_fitting)
            elif "Ar" in data.incident_atom:
                ar_data.append(conc_Au_fitting)
            elif "Kr" in data.incident_atom:
                kr_data.append(conc_Au_fitting)
        ...
        if data.incident_atom == "Ne":
            ...
            Kr_conc.append(conc_Au_fitting)
